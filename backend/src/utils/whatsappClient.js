...
const initWhatsApp = () => {
  ...
  initPromise = new Promise((resolve) => {
    ...
    client.on('qr', (qr) => {
      ...
      QRCode.toFile(qrImagePath, qr, { width: 200, margin: 1 }, (err) => {
        if (err) {
          logger.error('[WHATSAPP] Failed to save QR image: ' + err.message);
        } else {
          logger.info('[WHATSAPP] QR Code saved! Open this file to scan:');
          console.log('\n📱 WhatsApp QR Code saved as image:');
          console.log('   👉 ' + qrImagePath + '\n');
          console.log('   Open the file above and scan it with WhatsApp > Linked Devices\n');
        }
      });
    });
