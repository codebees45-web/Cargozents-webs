...
export default function DriverNotifications() {
  ...
  const [alerts, setAlerts] = useState([
    { id: 1, title: 'New Load Assigned', message: 'You have been assigned to Order #CB-9082 from Chennai to Bangalore. Tap for route details.', time: '10 mins ago', urgent: true },
    { id: 2, title: 'Route Update', message: 'Traffic congestion reported on NH-48. Suggesting an alternate route.', time: '1 hr ago', urgent: false },
    { id: 3, title: 'Document Verified', message: 'Your uploaded digital POD for your last trip has been approved.', time: '3 hrs ago', urgent: false },
  ]);
      ...
      <div className="max-w-4xl pb-10">
        {!notificationsEnabled ? (
          ...
          <div className="space-y-4">
            {alerts.map((notif) => (
              <div key={notif.id} className={`p-4 rounded-xl border bg-white shadow-sm flex items-start gap-4 border-l-4 transition-all duration-300 hover:shadow-md ${notif.urgent ? 'border-l-red-500' : 'border-l-blue-600'}`}>
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className="text-sm font-bold text-[#133C2C] flex items-center gap-2">
                      {notif.title}
                      {notif.urgent && <span className="px-1.5 py-0.5 text-[10px] bg-red-100 text-red-700 font-extrabold rounded animate-pulse">URGENT</span>}
                    </h4>
